/*
 * Simulador del procesador MIPS segmentado (pipeline de 5 etapas).
 *
 * Etapas: IF (fetch) -> ID (decode) -> EX (ALU) -> MEM (memoria) -> WB (write-back).
 *
 * Manejo de riesgos (hazards):
 *   - Datos: forwarding desde EX/MEM (1 instrucción adelante) y desde el banco
 *     de registros ya actualizado para las de 2 y 3 adelante (write_back corre
 *     antes que EX dentro de cpu_step).
 *   - Carga-uso (load-use): stall de 1 ciclo detectado en ID.
 *   - Control: los branches se resuelven en EX y se hace flush de IF/ID; los
 *     saltos j/jr se resuelven en ID con redirección del PC.
 *
 * Nota sobre cpu_step: las etapas se recorren en orden inverso (WB, MEM, EX,
 * ID, IF) para que cada etapa lea su latch de entrada antes de que la etapa
 * anterior lo sobrescriba, emulando la actualización simultánea del hardware.
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
typedef struct {
    ...
    uint32_t next_pc;
    bool branch_taken; // Redirección de PC pendiente (branch tomado en EX o jump en ID)
    bool stall;        // Congelar PC/IF-ID por riesgo carga-uso (load-use)
} MIPSPipeline;
...
void instruction_fetch(MIPSPipeline* cpu) {
    // Riesgo carga-uso: se congela el fetch un ciclo. Se preserva IF/ID y el PC
    // no avanza, de modo que la instrucción detenida se vuelva a decodificar.
    if (cpu->stall) {
        cpu->stall = false;
        return;
    }

    // Un salto tomado (branch resuelto en EX o jump/jr resuelto en ID) redirige el PC.
    if (cpu->branch_taken) {
        cpu->pc = cpu->next_pc;
...
void instruction_decode(MIPSPipeline* cpu) {
    // Riesgo de control: si un branch se resolvió tomado en EX en este mismo
    // ciclo (branch_taken ya activo antes de que corra IF), la instrucción que
    // está en IF/ID pertenece al camino equivocado y debe descartarse (flush).
    if (cpu->branch_taken) {
    ...
    // ¿Qué registros lee realmente esta instrucción? (para detectar load-use)
    bool uses_rs = (opcode == 0) || opcode == 8 || opcode == 35 ||
                   opcode == 43 || opcode == 4 || opcode == 5;
    bool uses_rt = (opcode == 0) || opcode == 43 || opcode == 4 || opcode == 5;

    // Riesgo carga-uso (load-use): la instrucción que está en EX es un lw cuyo
    // destino es fuente de esta instrucción. El dato de la carga solo estará
    // disponible tras MEM, así que se inserta una burbuja de 1 ciclo.
    if (cpu->id_ex.valid && cpu->id_ex.ctrl.mem_read) {
        uint8_t load_dst = cpu->id_ex.rt; // rt es el destino de lw
        if (load_dst != 0 &&
            ((uses_rs && load_dst == rs) || (uses_rt && load_dst == rt))) {
            cpu->id_ex.valid = false; // burbuja hacia EX en el próximo ciclo
            cpu->stall = true;        // IF preserva IF/ID y no avanza el PC
            return;
        }
    }
...
void alu_execute(MIPSPipeline* cpu) {
    ...
    // ---------------------------------------------------------------
    // UNIDAD DE FORWARDING (adelantamiento de datos)
    //
    // Se parte del banco de registros ya actualizado: write_back() corre antes
    // que esta etapa dentro de cpu_step(), por lo que regs[] ya contiene el
    // resultado de la instrucción que está 2 posiciones adelante (la que
    // termina WB en este ciclo) y de todas las anteriores. Eso cubre los
    // riesgos a distancia 2 y 3.
    //
    // El único caso pendiente es la instrucción que está 1 adelante (aún en
    // EX/MEM, todavía no escribió el banco): se adelanta su resultado de ALU
    // directamente. No se adelanta desde una carga (lw) porque su alu_out es la
    // dirección, no el dato; ese caso se resuelve con el stall de load-use.
    // ---------------------------------------------------------------
    uint32_t rs_val = cpu->regs[cpu->id_ex.rs];
    uint32_t rt_val = cpu->regs[cpu->id_ex.rt];

    if (cpu->ex_mem.valid && cpu->ex_mem.ctrl.reg_write &&
        !cpu->ex_mem.ctrl.mem_read && cpu->ex_mem.dest_reg != 0) {
        if (cpu->ex_mem.dest_reg == cpu->id_ex.rs) rs_val = (uint32_t)cpu->ex_mem.alu_out;
        if (cpu->ex_mem.dest_reg == cpu->id_ex.rt) rt_val = (uint32_t)cpu->ex_mem.alu_out;
    }
...
void test_integral_vectors() {
    ...
    TestVector vec_alu = {
        ...
        .instructions = {
            ((uint32_t)8 << 26) | (0 << 21) | (8 << 16) | 10,                 // addi $t0, $0, 10
            ((uint32_t)8 << 26) | (0 << 21) | (9 << 16) | 5,                  // addi $t1, $0, 5
            ((uint32_t)0 << 26) | (8 << 21) | (9 << 16) | (10 << 11) | 32,    // add $t2, $t0, $t1  (15)
            ((uint32_t)0 << 26) | (8 << 21) | (9 << 16) | (11 << 11) | 34,    // sub $t3, $t0, $t1  (5)
            ((uint32_t)0 << 26) | (8 << 21) | (9 << 16) | (12 << 11) | 36,    // and $t4, $t0, $t1  (0)
            ((uint32_t)0 << 26) | (8 << 21) | (9 << 16) | (13 << 11) | 37,    // or $t5, $t0, $t1   (15)
            ((uint32_t)0 << 26) | (8 << 21) | (9 << 16) | (14 << 11) | 38,    // xor $t6, $t0, $t1  (15)
        },
        .inst_count = 7,
    ...
    TestVector vec_mem = {
        ...
        .cycles = 10,
        .instructions = {
            ((uint32_t)8 << 26) | (0 << 21) | (8 << 16) | 42,                 // addi $t0, $0, 42
            ((uint32_t)43 << 26) | (0 << 21) | (8 << 16) | 4,                 // sw $t0, 4($0)
            ((uint32_t)35 << 26) | (0 << 21) | (9 << 16) | 4,                 // lw $t1, 4($0)
        },
        .inst_count = 3,
        .reg_exps = { {8, 42}, {9, 42} },
        .reg_exp_count = 2,
    ...
    TestVector vec_ctrl = {
        ...
        .cycles = 15,
        .instructions = {
            ((uint32_t)8 << 26) | (0 << 21) | (8 << 16) | 1,                  // 0: addi $t0, $0, 1
            ((uint32_t)4 << 26) | (8 << 21) | (8 << 16) | 2,                  // 4: beq $t0, $t0, 2 (Salta 2 intrucciones)
            ((uint32_t)8 << 26) | (0 << 21) | (9 << 16) | 99,                 // 8: addi $t1, $0, 99 (Ignorada por branch)
            ((uint32_t)8 << 26) | (0 << 21) | (10 << 16) | 99,                // 12: addi $t2, $0, 99 (Ignorada por branch)
            ((uint32_t)8 << 26) | (0 << 21) | (11 << 16) | 7,                 // 16: addi $t3, $0, 7 (Destino del branch)
            ((uint32_t)5 << 26) | (8 << 21) | (11 << 16) | 3,                 // 20: bne $t0, $t3, 3 (Salta porque 1 != 7)
            ((uint32_t)8 << 26) | (0 << 21) | (12 << 16) | 99,                // 24: addi $t4, $0, 99 (Ignorada)
        },
        .inst_count = 7,
